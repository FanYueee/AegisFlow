...
function renderControl(s){controlState=s;const live=s.config.mode==='live';$('bgpStatus').textContent=live?(s.bgp.connected?`已連線 · AS${s.bgp.asn} · Router ID ${s.bgp.router_id}`:`未連線 · ${s.bgp.error||'檢查中'}`):'尚未設定 GoBGP 連線';$('modeNotice').textContent=live?'實際模式：宣告與撤回會送至 GoBGP。':'先在下方設定 GoBGP 連線與允許宣告網段。';$('announce').textContent='宣告路由';$('announce').disabled=!live||!s.bgp.connected;table('peers',(s.bgp.peers||[]).map(p=>[p.address,p.asn,p.state]),live?'尚無鄰居':'尚未連線');
...
const panels=[[14,'介面頻寬'],[15,'介面封包'],[1,'總頻寬'],[2,'每秒封包'],[3,'累計流量'],[4,'每秒樣本'],[5,'protocol 頻寬'],[6,'protocol 封包'],[8,'目的網段頻寬'],[9,'目的網段封包'],[12,'tcp flag 頻寬'],[13,'tcp flag']];
...
function updatePanels(tcpOnly=false){
 ...
 $('interfaceDetails').hidden=!selected;
 $('interfaceHeading').textContent=selected?$('interfaceSelect').selectedOptions[0].textContent+' · '+$('directionSelect').value:'';
 const params=new URLSearchParams({orgId:'1',from:$('chartRange').value,to:'now',refresh:'5s','var-tcp_scope':$('tcpScope').value,'var-interface':selected||'$__all','var-direction':$('directionSelect').value,theme:'light'});
 const dashboard=new URL(grafana);dashboard.search=params.toString();$('grafana').href=dashboard.href;
 for(const frame of document.querySelectorAll('[data-panel-id]')){const id=Number(frame.dataset.panelId);if(tcpOnly&&![12,13].includes(id))continue;if(id<14&&!selected){frame.removeAttribute('src');continue}const url=new URL(dashboard);url.pathname='/d-solo/aegisflow-overview';url.searchParams.set('panelId',id);if(id>=14){url.searchParams.set('var-interface',selected||'$__all');url.searchParams.set('var-direction','IN')}if(frame.getAttribute('src')!==url.href)frame.src=url.href}
}
let interfaceSignature='',initialInterface=new URLSearchParams(location.search).get('interface')||'netflow9@172.30.10.1@15';
...
async function pollInterfaces(){try{
 const rows=await api('/api/interfaces');const signature=JSON.stringify(rows);
 if(signature!==interfaceSignature){interfaceSignature=signature;const select=$('interfaceSelect'),current=select.value;select.replaceChildren(new Option('所有介面總覽',''));$('interfaceButtons').replaceChildren();
 for(const row of rows){const label=row.exporter+' / '+(row.index==='0'?'未知介面 (0)':'介面 '+row.index)+' · '+row.source;select.add(new Option(label,row.key));const button=document.createElement('button');button.type='button';button.textContent=label;button.addEventListener('click',()=>{select.value=row.key;updatePanels();$('interfaceDetails').scrollIntoView({behavior:'smooth',block:'start'})});$('interfaceButtons').append(button)}
 if(rows.some(r=>r.key===current))select.value=current;else if(initialInterface&&rows.some(r=>r.key===initialInterface)){select.value=initialInterface;initialInterface=null}updatePanels()}
 $('interfaceStatus').textContent=rows.length?'點選介面查看詳細資料；IN／OUT 以設備介面為準。':'等待設備回報介面。';
 }catch(e){$('interfaceStatus').textContent='介面清單讀取失敗：'+e.message}finally{setTimeout(pollInterfaces,5000)}}
$('chartRange').addEventListener('change',()=>updatePanels());
$('tcpScope').addEventListener('change',()=>updatePanels(true));
$('interfaceSelect').addEventListener('change',()=>updatePanels());
$('directionSelect').addEventListener('change',()=>updatePanels());
updatePanels();pollInterfaces();pollControl();
